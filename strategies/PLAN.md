...
| 36 ❌ | Crash filter overlay on #35 (VIX + self-momentum, identical spec to #22/#29) | VIX 20→40 linear scaling, self-mom gate at z(tr20d) < −1σ → 0.5, combined multiplicatively; recomputed cost. | ❌ **Second cross-spec rejection of the overlay's universality (after #30).** 1/4 pre-registered criteria pass. Sharpe 1.44 → 1.45 (statistically flat), but IR **−0.64**, MaxDD only 1.4pp better. **Conditional Sharpe FAILS**: on filter-binding days (1,377 = 38% of sample) base earned +1.03 Sharpe, filtered +0.81 → filter removed GOOD days. VIX-vs-crypto decorrelation is the root cause: 2018 crypto winter happened at low VIX, 2022 crypto crash coincided with a rate-hike environment not equity panic. Sub-period: 2018 −0.40, 2022 −0.52 Sharpe damage. Silver lining: skew flipped −0.07 → +0.26. **Overlay scoreboard: #22 ✅, #29 ✅, #30 ❌, #36 ❌.** Deployment rule doubly confirmed. |
| **37** ✅ | **Parameter robustness sweep on #35** | 6 lookbacks × 4 top-N grid (24 backtests total). LB ∈ {30, 60, 90, 120, 180, 252}, N ∈ {2, 3, 4, 5}. Same universe, same monthly rebal, same 30 bps RT cost. Pre-registered 4-criterion pass gate. | ✅ **All 24 cells pass.** Sharpe range **+1.16 to +1.44**, median **+1.35**, 100% of cells above 1.0. Baseline neighbours ≥ +1.16 → +1.44 is the peak of a broad plateau, not a spike. Rules out overfit as the explanation for #35's win. Materially strengthens the crypto-cross-sectional-momentum finding — the edge is a genuine cross-sectional momentum anomaly in crypto, not a lucky parameter pick. Recommended next step: sub-period stability check (regime-by-regime Sharpe across all 24 variants) and/or apply crypto-native crash filter (BTC drawdown gate) since #36 showed VIX doesn't work. |
| **38** ✅⚠ | **Nifty 100 Low-Vol 30 — first Indian equities strategy** | Universe: 100 Nifty large caps (`.NS` yfinance). Signal: trailing 252d realised vol. Long bottom-30, equal-weight, annual Jan rebal. Cost: 20 bps RT (STT + brokerage). Benchmark: Nifty 50 B&H + EW-universe passive. | ✅ **First cited use of research/india_equities.md §1.2.** Net Sharpe **1.34** vs Nifty 50 **0.71**; CAGR 17.09% vs 10.75%; MDD −27.3% vs −38.4%. 3/4 pre-registered criteria pass. ⚠ **BUT**: (a) universe is current Nifty 100 (survivorship-biased) — doc's cited 0.47 Sharpe uses point-in-time NSE data, likely explaining the ~3× Sharpe inflation; (b) IR vs EW universe = **−0.42** — equal-weight earns more (CAGR 19.79%); the source of alpha is the size/EW tilt vs cap-weighted N50, NOT the vol ranking. Deployable as defensive Indian large-cap vehicle, NOT a factor validation of low-vol anomaly. Next: sector 12-1 momentum on SPDR sectors (US) or Nifty 200 12-1 momentum (India) to see if a genuine cross-sectional ranking works where low-vol didn't. |
